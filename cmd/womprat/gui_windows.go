...
// uintptrToPointer converts a Win32 handle (HWND as uintptr) to unsafe.Pointer
// for the WebView2 Window option without the direct uintptr->Pointer conversion
// that go vet flags. Window handles are opaque identifiers, not Go pointers.
...
func runGUI(app *App, shellURL string) {
	...
	w := webview2.NewWithOptions(webview2.WebViewOptions{Debug: app.config.DebugLog, AutoFocus: true, Window: uintptrToPointer(shellChild), DataPath: webviewDataPath()})
	if w == nil {
