...
func TestUpdatePDU_Deserialize(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			// header: 0x00 (action=0, flags=0), total length: 0x07, data: 0x01,0x02,0x03,0x04,0x05
			input:          []byte{0x00, 0x07, 0x01, 0x02, 0x03, 0x04, 0x05},
		...
		},
		{
			...
			// header: 0x00, total length: 0x80 0x88 (2-byte length = 0x0088 = 136)
			input:          append([]byte{0x00, 0x80, 0x88}, make([]byte, 133)...),
			expectedAction: UpdatePDUActionFastPath,
			expectedFlags:  0,
			expectedLen:    133,
			expectedErr:    nil,
		},
		{
			...
			// header: 0x03 (action=3 = X224), total length: 0x02 (empty payload)
...
func TestUpdatePDU_Deserialize_ReadFullOnChunkedReader(t *testing.T) {
	...
	reader := &shortChunkReader{
		data:  append([]byte{0x00, byte(len(payload) + 2)}, payload...),
		chunk: 1,
	}
	pdu := &UpdatePDU{}
...
func TestProtocol_Receive(t *testing.T) {
	...
	input := append([]byte{0x00, 0x07}, data...)
...
func TestRoundTrip_UpdatePDU(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "bitmap update",
			data:    append([]byte{0x00, 0x0a}, make([]byte, 8)...), // action=0, total len=10, 8 bytes data
		...
		{
			name:    "synchronize update",
			data:    []byte{0x00, 0x04, 0x00, 0x00}, // action=0, total len=4, 2 bytes sync payload
			action:  UpdatePDUActionFastPath,
			flags:   0,
			dataLen: 2,
		},
