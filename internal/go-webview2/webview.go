...
type browser interface {
	...
	Resize()
	ResizeTo(x int32, y int32, width int32, height int32)
	Navigate(url string)
...
func (w *webview) SetBounds(x int, y int, width int, height int) {
	if width < 0 {
		width = 0
	}
	if height < 0 {
	...
	w.browser.ResizeTo(int32(x), int32(y), int32(width), int32(height))
}
